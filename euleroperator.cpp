...
* we must specify all components in terms of density \f$\rho$, momentum $\rho
* \mathbf{u} \f$ and energy E. Given this information, we then store the
* function alongside the respective boundary id in a map member variable of
* this class. For the present DG code where boundary conditions are solely applied
* as part of the weak form (during time integration), the call to set the boundary
* conditions can appear both before or after the `reinit()` call to this class.
*
...
/*! Likewise, we proceed for the subsonic outflow boundaries (where
* we request a function as well, which we use to retrieve the energy)*/
template <int dim, int degree, int n_points_1d>
...
* associate the variable `phi_m` with the solution component \f$\mathbf{w}^- \f$
* and the variable `phi_p` with the solution component \f$\mathbf{w}^+ \f$. We
...
* the Gauss-Lobatto quadrature formula, only \f$(p+1)^{d-1} \f$ out of the
* \f$(p+1)^d \f$ basis functions evaluate to non-zero on each face. Thus, the
...
* values and derivatives, indeed all \f$(p+1)^d \f$ vector entries for each
...
* evaluated at quadrature points from both sides (i.e., \f$\mathbf{w}^- \f$ and
* \f$\mathbf{w}^+ \f$), as well as the normal vector onto the minus side. As
...
* <i>exterior</i> quantity \f$\mathbf{w}^+ \f$ that is then handed to the
...
* energy with \f$\rho^+ = \rho^- \f$ and \f$E^+ = E^- \f$. To achieve the no-normal
...
* \f$\mathbf{w}_\mathrm{D} \f$ as a boundary value. An alternative would have been
* to use \f$\mathbf{w}^+ = -\mathbf{w}^- + 2 \mathbf{w}_\mathrm{D} \f$, the
* so-called mirror principle.
* The imposition of outflow is essentially a Neumann condition, i.e.,
* setting \f$\mathbf{w}^+ = \mathbf{w}^- \f$.
...
* different interpolation matrix, representing the inverse \f$S^{-1} \f$
...
* operator as a whole, i.e., \f$\mathcal M^{-1} \mathcal L(t, \mathbf{w}) \f$,
...
* Once the spatial operator \f$\mathcal L \f$ is applied, we need to make a
...
* shape functions evaluated at quadrature points by \f$S \f$ , the projection on
* cell \f$K \f$ is an operation of the form \f$\underbrace{S J^K S^\mathrm
* T}_{\mathcal M^K} \mathbf{w}^K = S J^K
* \tilde{\mathbf{w}}(\mathbf{x}_q)_{q=1:n_q} \f$, where \f$J^K \f$ is the diagonal
* matrix containing the determinant of the Jacobian times the quadrature
* weight (JxW), \f$\mathcal M^K \f$ is the cell-wise mass matrix, and
* \f$\tilde{\mathbf{w}}(\mathbf{x}_q)_{q=1:n_q} \f$ is the evaluation of the
...
* \f$\mathbf{w}^K = \left(S J^K S^\mathrm T\right)^{-1} S J^K
* \tilde{\mathbf{w}}(\mathbf{x}_q)_{q=1:n_q} = S^{-\mathrm T}
* \left(J^K\right)^{-1} S^{-1} S J^K
* \tilde{\mathbf{w}}(\mathbf{x}_q)_{q=1:n_q} \f$. Now, the term \f$S^{-1} S \f$ and
* then \f$\left(J^K\right)^{-1} J^K \f$ cancel, resulting in the final
* expression \f$\mathbf{w}^K = S^{-\mathrm T}
* \tilde{\mathbf{w}}(\mathbf{x}_q)_{q=1:n_q} \f$.
...
* the multiplication by \f$S^{-\mathrm T} \f$, a basis change from the
...
* lane of the vectorized array holds data from a different cell.*/
template <int dim, int degree, int n_points_1d>
...
* velocity \f$\mathbf{u} \f$ and the propagation of sound waves with sound
* speed \f$c = \sqrt{\gamma p/\rho} \f$ relative to the medium moving at
...
* the medium, \f$\mathbf u \f$ is scaled by the mesh size,
* so an estimate of the maximal velocity can be obtained by computing
* \f$\|J^{-\mathrm T} \mathbf{u}\|_\infty \f$, where \f$J \f$ is the Jacobian of the
...
* by the minimal singular value of \f$J \f$ or, equivalently, the maximal
* singular value of \f$J^{-1} \f$. Note that one could approximate this quantity
